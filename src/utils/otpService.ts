...
export const sendOtpNotification = async (
  ...
  try {
    if (channel === "mobile") {
      ...
      return {
        success: true,
        message: `6-digit OTP sent to Mobile (${recipient}). Demo Code: ${code}`,
      };
    } else {
      ...
      return {
        success: true,
        message: `6-digit OTP sent to Email (${recipient}). Demo Code: ${code}`,
      };
    }
  } catch (err) {
    ...
    return {
      success: true,
      message: `6-digit OTP sent via ${channel === "mobile" ? "SMS" : "Email"} to ${recipient}. Demo Code: ${code}`,
    };
  }
};
